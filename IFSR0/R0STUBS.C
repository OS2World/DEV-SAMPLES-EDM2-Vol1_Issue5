...
short int far pascal _export _loadds FS_ATTACH(unsigned short flag, 
   ...
   // Verify parameters
   if (flag == FSA_ATTACH || flag == FSA_DETACH) {
      rc = FSH_PROBEBUF(PB_OPREAD, pParm, *pLen);
   } else if (flag == FSA_ATTACH_INFO) {
   ...
   // From here to the FSH_SEMSET(&CPData.CmdComplete), we MUST NOT block
...
#pragma argsused
short int far pascal _export _loadds FS_ALLOCATEPAGESPACE(
...
#pragma argsused
short int far pascal _export _loadds FS_DOPAGEIO(
    struct sffsi far *         psffsi,      /* ptr to fs independent SFT    */
    struct sffsd far *         psffsd,      /* ptr to fs dependent SFT      */
...
#pragma argsused
short int far pascal _export _loadds FS_OPENPAGEFILE (
